...
import co.crowtec.jitsi.JitsiPlugin.Companion.JITSI_MEETING_CLOSE
 ...
 */
class JitsiPluginActivity : JitsiMeetActivity() {
    companion object {
        @JvmStatic
        fun launchActivity(context: Context?,
                           options: JitsiMeetConferenceOptions) {
            var intent = Intent(context, JitsiPluginActivity::class.java).apply {
    ...
    override fun onPictureInPictureModeChanged(isInPictureInPictureMode: Boolean, newConfig: Configuration?) {
        ...
        if (isInPictureInPictureMode){
            JitsiEventStreamHandler.instance.onPictureInPictureWillEnter()
        }
        else {
            JitsiEventStreamHandler.instance.onPictureInPictureTerminated()
        }
    ...
    override fun onConferenceWillJoin(data: HashMap<String, Any>) {
        Log.d(JITSI_PLUGIN_TAG, String.format("JitsiPluginActivity.onConferenceWillJoin: %s", data))
        JitsiEventStreamHandler.instance.onConferenceWillJoin(data)
        super.onConferenceWillJoin(data)
    }

    override fun onConferenceJoined(data: HashMap<String, Any>) {
        Log.d(JITSI_PLUGIN_TAG, String.format("JitsiPluginActivity.onConferenceJoined: %s", data))
        JitsiEventStreamHandler.instance.onConferenceJoined(data)
        super.onConferenceJoined(data)
    }

    override fun onConferenceTerminated(data: HashMap<String, Any>) {

        Log.d(JITSI_PLUGIN_TAG, String.format("JitsiPluginActivity.onConferenceTerminated: %s", data))
        JitsiEventStreamHandler.instance.onConferenceTerminated(data)
